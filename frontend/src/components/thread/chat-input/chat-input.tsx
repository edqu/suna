...
export const ChatInput = memo(
  forwardRef<ChatInputHandles, ChatInputProps>(
    (
      {
        onSubmit,
      ...
      const [sunaAgentModes, setSunaAgentModes] = useState<
        'adaptive' | 'autonomous' | 'chat'
      >('adaptive');
      ...
      const shouldShowUsage = useMemo(() => {
        if (!subscriptionData || !showToLowCreditUsers || isLocalMode())
          return false;
      ...
      const generateDataOptionsMarkdown = useCallback(() => {
        if (
          selectedMode !== 'data' ||
          (selectedCharts.length === 0 && !selectedOutputFormat)
        ) {
          return '';
        }
      ...
      const handleSubmit = useCallback(
        ...
        [
          ...
          isUploading,
          onStopAgent,
          generateDataOptionsMarkdown,
          generateSlidesTemplateMarkdown,
          getActualModelId,
          selectedModel,
          onSubmit,
          selectedAgentId,
        ...
        return (
          <div className="flex items-center gap-2">
            <UnifiedConfigMenu
              ...
              selectedAgentId={
                !hideAgentSelection ? selectedAgentId : undefined
              }
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent
                          align="start"
                          className="w-[320px] px-0 py-3 border-[1.5px] border-border rounded-2xl"
                          sideOffset={6}
                        >
                          ...
                          <div className="space-y-0.5 px-2">
                            {quickIntegrations.map((integration) => (
                              ...
                              >
                                ...
                                >
                                  <div
                                    className="flex items-center justify-center w-8 h-8 bg-card border-[1.5px] border-border flex-shrink-0"
                        ...
                        <TooltipContent side="bottom">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-gray-200">
                              Quick responses with smart context switching
                            </p>
                          </div>
                        </TooltipContent>
                      </Tooltip>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent side="bottom">
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-gray-200">
                              Deep work mode for multi-step problem solving
                            </p>
                          </div>
                        </TooltipContent>
                      </Tooltip>
          ...
          <div className="relative">
                  ...
                  <div className="flex items-center justify-between gap-1 overflow-x-auto scrollbar-none relative">
                    ...
                    >
                      <div className="flex items-center -space-x-0.5">
                        {quickIntegrations.every(
                          (int) =>
                            integrationIcons[
                              int.id as keyof typeof integrationIcons
                            ],
                        ) ? (
                          <>
                            {quickIntegrations.map((integration) => (
                              <div
                                key={integration.id}
                                className="w-4 h-4 bg-white dark:bg-muted border border-border rounded-full flex items-center justify-center shadow-sm"
                              >
                    ...
                    <button
                      onClick={() =>
                        setAgentConfigDialog({ open: true, tab: 'tools' })
                      }
                      className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-all duration-200 px-2.5 py-1.5 rounded-lg hover:bg-muted/50 border border-transparent hover:border-border/30 flex-shrink-0 cursor-pointer relative pointer-events-auto"
                    >
                    ...
                    <button
                      onClick={() =>
                        setAgentConfigDialog({ open: true, tab: 'knowledge' })
                      }
                      className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-all duration-200 px-2.5 py-1.5 rounded-lg hover:bg-muted/50 border border-transparent hover:border-border/30 flex-shrink-0 cursor-pointer relative pointer-events-auto"
                    >
                    ...
                    <button
                      onClick={() =>
                        setAgentConfigDialog({ open: true, tab: 'triggers' })
                      }
                      className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-all duration-200 px-2.5 py-1.5 rounded-lg hover:bg-muted/50 border border-transparent hover:border-border/30 flex-shrink-0 cursor-pointer relative pointer-events-auto"
                    >
            ...
            {selectedAgentId && agentConfigDialog.open && (
              <AgentConfigurationDialog
                ...
                onOpenChange={(open) =>
                  setAgentConfigDialog({ ...agentConfigDialog, open })
                }
