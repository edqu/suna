...
function preprocessTextOnlyTools(content: string): string {
  ...
  content = content.replace(
    /<function_calls>\s*<invoke name="present_presentation">[\s\S]*?<parameter name="text">([\s\S]*?)<\/parameter>[\s\S]*?<\/invoke>\s*<\/function_calls>/gi,
    '$1',
  );
  ...
  content = content.replace(
    /<function_calls>\s*<invoke name="present_presentation">[\s\S]*?<parameter name="text">([\s\S]*?)$/gi,
    '$1',
  );
  ...
  content = content.replace(
    /<present_presentation[^>]*>([\s\S]*?)<\/present_presentation>/gi,
    '$1',
  );
  return content;
}
          ...
          contentParts.push(
            <div key={`tool-${match.index}-${index}`} className="my-1">
              ...
              >
                ...
                {paramDisplay && (
                  <span
                    className="ml-1 text-foreground/60 truncate max-w-[200px]"
                    title={paramDisplay}
                  >
                    {paramDisplay}
                  </span>
...
export const ThreadContent: React.FC<ThreadContentProps> = ({
  ...
  const displayMessages = useMemo(() => {
    ...
    return allMessages.filter((msg) => {
      ...
      // Filter out tool_call_chunk status messages - they're internal streaming state
      if (msg.type === 'status') {
        const parsedContent = safeJsonParse<ParsedContent>(msg.content, {});
              ...
              {(() => {
                ...
                return finalGroupedMessages.map((group, groupIndex) => {
                  if (group.type === 'user') {
                    ...
                    const groupAgent = groupAgentId
                      ? agentsMap.get(groupAgentId)
                      : undefined;
